...
- **API Intelligence**: Detailed API documentation, endpoint descriptions, and sequence diagrams for complex logic must be deposited in `/docs/architecture/`.
- **Integration Specs**: Documentation for all third-party integrations and service dependencies must be deposited in `/docs/architecture/integrations/`.
- **Verification**: Your `verify.md` artifact must be deposited in `/docs/verification/` showing that the implementation perfectly matches the API contract and passes security audits.
